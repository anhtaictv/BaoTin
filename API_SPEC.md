...
## Camera gần hiện trường (bổ sung Giai đoạn 1)
- `GET /officer/reports/:id/nearby-cameras?radius=300` — tìm camera trong bán kính (mét) quanh vị trí report, sắp theo khoảng cách gần nhất
- `POST /officer/reports/:id/footage-requests`
  - Body: `{ camera_id, time_window_start, time_window_end, notes }`
  - Tạo phiếu yêu cầu trích xuất, KHÔNG tự động lấy hay xử lý video
- `PATCH /footage-requests/:id/status` — cập nhật trạng thái (`contacted`/`received`/`no_response`/`declined`), gắn `footage_file_url` nếu nhận được file
- `GET /officer/reports/:id/footage-requests` — xem toàn bộ yêu cầu camera đã tạo cho tin này
