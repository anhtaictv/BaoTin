...
<p align="center">
  <img alt="backend" src="https://img.shields.io/badge/backend-1.27.0-2563eb?style=flat-square&logo=nodedotjs&logoColor=white">
  <img alt="dashboard-web-react" src="https://img.shields.io/badge/dashboard--web--react-0.8.0-2563eb?style=flat-square&logo=react&logoColor=white">
  ...
  <img alt="mobile-app-officer" src="https://img.shields.io/badge/mobile--app--officer-1.18.0%2B26-2563eb?style=flat-square&logo=flutter&logoColor=white">
  <img alt="mobile-app-citizen" src="https://img.shields.io/badge/mobile--app--citizen-1.13.0%2B20-2563eb?style=flat-square&logo=flutter&logoColor=white">
</p>
<p align="center">
...
npx vitest run                         # 825 test: crypto, validation, geo-matching, auth/report/
...
| Version | Nội dung |
|---|---|
| **v1.27** | Tính năng "Trưởng xã" (`commune_head`): phân địa bàn xã/phường mới (sau sáp nhập) thành các địa bàn con theo ranh giới xã/phường **cũ** (trước 2025) rồi gán từng địa bàn con cho cán bộ cấp dưới — trang "Phân địa bàn theo xã/phường cũ" mới trên `dashboard-web-react` và `mobile-app-officer`, đọc/ghi qua `officer/commune` API mới; seed dữ liệu ranh giới cũ Đắk Lắk + Phú Yên (geojson gốc), % chồng lấn giữa địa bàn cũ/mới tính bằng PostGIS thật (`ST_Intersection`/`ST_Area`); vá lỗi tài khoản `commune_head` đăng nhập đúng mật khẩu vẫn báo sai — route self-service `/web-accounts/me` (gọi ngay sau khi login) thiếu role này trong danh sách cho phép; vá pipeline deploy Windows PowerShell mangle tên file geojson có dấu tiếng Việt |
| **v1.26** | SOS reliability & backend perf: hàng đợi SOS offline tự retry ở `mobile-app-citizen` (idempotency, phản hồi UI rõ ràng), officer polling 30s kèm âm báo hệ thống, rate limiting + graceful shutdown + cache dashboard + index truy vấn ở backend, health-check deploy; **[1.26.1]** chế độ "Chữ to, dễ đọc" cho người lớn tuổi ở `mobile-app-citizen` (phóng to `MediaQuery.textScaler` toàn app, lưu `shared_preferences`); bản đồ nhiệt cho "Bản đồ tin báo" ở `dashboard-web-react` (toggle marker/heatmap, dùng lại dữ liệu đã fetch); chore backend — AI Gateway dùng chung (WebApp/BaoTin/Soul Diary) làm default cho `OPENAI_BASE_URL` khi `LLM_PROVIDER=openai`, mặc định vẫn "none" nên không đổi hành vi hiện tại |
...
| **v1.26** | SOS reliability & backend perf: hàng đợi SOS offline tự retry ở `mobile-app-citizen` (`emergency_queue.dart`, idempotency + phản hồi UI rõ khi mất mạng), `mobile-app-officer` polling 30s kèm âm báo hệ thống khi có SOS mới; backend: rate limiting cho luồng report, graceful shutdown, cache dashboard stats, thêm index truy vấn, health-check script cho pipeline deploy; **[1.26.1]** chế độ "Chữ to, dễ đọc" cho người lớn tuổi ở `mobile-app-citizen` — toggle ở màn Hồ sơ, phóng to `MediaQuery.textScaler` toàn app, lưu qua `shared_preferences`; bản đồ nhiệt cho "Bản đồ tin báo" ở `dashboard-web-react` — toggle marker/heatmap (`leaflet.heat`), tái dùng đúng dữ liệu `report-locations` đã fetch sẵn; chore backend — AI Gateway dùng chung (WebApp/BaoTin/Soul Diary) làm default cho `OPENAI_BASE_URL` khi `LLM_PROVIDER=openai` (mặc định vẫn "none", không đổi hành vi hiện tại) |
| **v1.27** | Tính năng "Trưởng xã" (`commune_head`): chia nhỏ xã/phường mới thành các địa bàn con theo ranh giới xã/phường cũ (trước sáp nhập 2025) và gán từng địa bàn con cho cán bộ cấp dưới — trang mới trên `dashboard-web-react`/`mobile-app-officer`, API mới `officer/commune` (đọc: mọi role officer; ghi: `commune_head`/`admin`, ownership enforce ở service layer); seed 294 xã/phường cũ (Đắk Lắk + Phú Yên, từ geojson gốc trước sáp nhập) và 345 cặp chồng lấn cũ↔mới tính bằng PostGIS thật; vá lỗi tài khoản `commune_head` nhập đúng mật khẩu vẫn báo "sai tên đăng nhập/mật khẩu" — `AuthContext` gọi `GET /web-accounts/me` ngay sau khi login nhưng route đó thiếu role `commune_head` trong danh sách cho phép, chặn đứng toàn bộ tính năng ngay từ bước đăng nhập; vá pipeline deploy Windows PowerShell 5.1 đọc sai codepage làm mangle tên file geojson có dấu tiếng Việt |
