...
class _NfcCccdMockScreenState extends State<NfcCccdMockScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Quét CCCD (mô phỏng)')),
      body: Padding(
        ...
        FilledButton(
          onPressed: () {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('Đã ghi nhận đối chiếu danh tính (mô phỏng).')),
            );
            Navigator.of(context).pop();
          },
          child: const Text('Xác nhận đã đối chiếu danh tính'),
        ),
